  ...
  if (!disease) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-20 text-center">
        ...
        <Button asChild variant="medical"><Link to="/diseases">Back to Diseases</Link></Button>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
      <Button asChild variant="ghost" size="sm" className="mb-6 gap-2">
        <Link to="/diseases"><ArrowLeft className="w-4 h-4" />Back to Library</Link>
      </Button>
        ...
        <div className="relative h-64">
          ...
          <div className="absolute bottom-6 left-6 right-6">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <h1 className="text-3xl font-black text-white">{disease.name}</h1>
                <div className="flex gap-2 mt-1">
                  <Badge variant={severityColor[disease.severity]}>{disease.severity} severity</Badge>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="mb-8">
        <div className="p-5 rounded-2xl bg-muted/50 border border-border">
          <p className="text-foreground leading-relaxed">{disease.shortDescription}</p>
          <p className="text-sm text-muted-foreground mt-2 flex items-center gap-1.5">
            <span className="w-1.5 h-1.5 rounded-full bg-medical-cyan" /> {disease.prevalence}
          </p>
        </div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
        <Tabs defaultValue="symptoms">
          <TabsList className="w-full grid grid-cols-4 h-auto p-1.5 bg-muted rounded-2xl">
            {sections.map(s => (
              <TabsTrigger key={s.tab} value={s.tab} className="text-xs sm:text-sm py-2 rounded-xl">{s.label}</TabsTrigger>
            ))}
          </TabsList>
          {sections.map(s => (
            <TabsContent key={s.tab} value={s.tab}>
              <motion.ul initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-3 mt-2">
                {s.items.map((item, i) => (
                  <motion.li key={item} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
      ...
      {/* CTA */}
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="mt-10 p-6 rounded-2xl bg-medical-blue text-white text-center">
        <h3 className="text-xl font-bold mb-2">Think you might have {disease.name}?</h3>
        <p className="text-white/70 text-sm mb-4">Upload your scan and get an AI-powered diagnosis in seconds.</p>
        <div className="flex gap-3 justify-center">
          <Button asChild variant="cyan"><Link to="/dashboard">Upload Scan</Link></Button>
          <Button asChild className="bg-white/10 text-white border border-white/30 hover:bg-white/20"><Link to="/find-doctors">Find Specialist</Link></Button>
        </div>
      </motion.div>
    </div>
  );
